...
#define iWAmacro_CONFIG_INI_FILENAME_LENGTH   (128)
#define iWAmacro_CONFIG_STRING_DEFAULT_VALUE    "stringvaluedefault"
#define iWAmacro_CONFIG_INTEGER_DEFAULT_VALUE    (-12345)

static iWAint8 ini_filename[iWAmacro_CONFIG_INI_FILENAME_LENGTH];


void iWA_Config_Init(iWAint8 *inifile)
{
    if(inifile)     iWA_Std_strcpy(ini_filename, inifile);

#if 0
    void test_minini(void);
    test_minini();
#endif
}

iWAbool iWA_Config_GetString(iWAint8 *section, iWAint8 *key, iWAint8 *buf, iWAuint32 buf_len)
{
    if(section == NULL || key == NULL || buf == NULL)  return 0;

    ini_gets(section, key, iWAmacro_CONFIG_STRING_DEFAULT_VALUE, buf, buf_len, ini_filename);

    return !!iWA_Std_strcmp(buf, iWAmacro_CONFIG_STRING_DEFAULT_VALUE);
}

iWAbool iWA_Config_GetInteger(iWAint8 *section, iWAint8 *key, iWAint32 *value)
{
    iWAint32 i;

    if(section == NULL || key == NULL || value == NULL)  return 0;

    *value = ini_getl(section, key, iWAmacro_CONFIG_INTEGER_DEFAULT_VALUE, ini_filename);

    return (i != iWAmacro_CONFIG_INTEGER_DEFAULT_VALUE);
}


#if 0         /* test case, copy from minIni package */
